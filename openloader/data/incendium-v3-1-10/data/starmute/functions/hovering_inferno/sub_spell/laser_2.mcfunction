...
execute as @e[type=blaze,tag=inferno] at @s if predicate starmute:80_percent run playsound block.respawn_anchor.deplete master @a[distance=..50] ~ ~ ~ 1 1.75 1
...
execute if score %inferno in.laser_time matches ..15 run schedule function starmute:hovering_inferno/sub_spell/laser_2 2t