...
advancement revoke @s only helth:eating/poisonous_potato_adv

# Food and Health points given:
#	+1 Food
#	-0.5 Heart
execute if score experimentalRegenEnabled StooSettings matches 1 run effect give @s poison 1 1 true
scoreboard players add @s StooFood 1
scoreboard players set @a[ scores={StooFood=42..} ] StooFood 41

# Slowness effect for 7 seconds
effect give @s slowness 7 0
