...
# HEALTH - Light HP boost for mobs listed in the entity tag file
effect give @e[type=#hardness:health_buffed_by_40_percent] resistance 10000 0 true
...
##### SPAWNS
# PHANTOMS - Homeworld
execute if score defeatedDragon StooSettings matches 1 if entity @a[predicate=hardness:in_end] if predicate math:random_chance_07 at @e[type=enderman, predicate=hardness:in_end, limit=1] run summon phantom ~ ~70 ~ {Tags:["fantomed"], Size:5, Health:12.0f}
...
# Reschedule
schedule function hardness:mob_clock_minute_challenge 60s
